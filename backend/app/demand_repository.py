...
class DemandRepository:
    ...
    @staticmethod
    def _normalize_lifecycle(data: dict[str, Any]) -> dict[str, Any]:
        """Padroniza encerramento operacional da OS antes de gravar.

        Na operação do SISAVALIA, uma OS marcada como finalizada já saiu da
        fila de prazo. Por isso ela é persistida como Entregue, com o sistema
        concluído e data de finalização preenchida.
        """
        status = data.get("demand_status")
        if status in {"Finalizada", "Entregue"}:
            data["demand_status"] = "Entregue"
            data["system_status"] = "Concluída"
            data["system_finished_at"] = data.get("system_finished_at") or date.today()
        elif status == "Cancelada":
            data["system_status"] = "Cancelada"
            data["system_finished_at"] = data.get("system_finished_at") or date.today()
        return data
    ...
    def create_demand(self, payload: DemandInput, *, user_id: str | None = None) -> dict[str, Any]:
        ...
        with connect(self.database_url) as connection, connection.cursor(row_factory=dict_row) as cursor:
            ...
            data = self._normalize_lifecycle(data)
            try:
                cursor.execute(
                    """
                    INSERT INTO demands
        ...
        with connect(self.database_url) as connection, connection.cursor(row_factory=dict_row) as cursor:
            cursor.execute(
                ...
                SELECT
                  ...
                  COUNT(*) FILTER (
                    WHERE CURRENT_DATE >= client_deadline
                      AND system_finished_at IS NULL
                      AND demand_status NOT IN ('Finalizada', 'Entregue', 'Cancelada', 'Não entregue')
                  ) AS overdue,
