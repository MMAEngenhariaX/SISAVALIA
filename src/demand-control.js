(() => {
  ...
  function renderDemandRows(items) {
    ...
    tableBody.innerHTML = items.map((item) => `
      <tr class="${item.deadline_status === "Fora do prazo" ? "demand-overdue" : ""}">
        ...
        <td>
          <select class="table-status-select demand-status-select" data-update-demand-status="${item.id}" aria-label="Atualizar status da OS ${escapeHtml(item.os_number)}">
            ${statusOptions(DEMAND_STATUS_OPTIONS, item.demand_status)}
          </select>
          <small>${item.deadline_status === "Fora do prazo" ? "Atenção: fora do prazo" : "Prazo regular"}</small>
        </td>
  ...
  function renderEvaluationDemandMatches(items) {
    ...
    evaluationDemandMatches.innerHTML = items.map((item) => `
      <div class="evaluation-demand-match">
        ...
        <div class="evaluation-demand-actions">
          <label>Status da OS
            <select class="table-status-select" data-update-search-demand-status="${item.id}" aria-label="Atualizar status da OS ${escapeHtml(item.os_number)} pela aba OS">
              ${statusOptions(DEMAND_STATUS_OPTIONS, item.demand_status)}
            </select>
          </label>
  ...
  async function quickUpdateDemand(demandId, overrides, control, successText = "Demanda atualizada com sucesso.") {
    ...
    if (!demand) return;
    const originalValue = control?.dataset?.updateDemandStatus ? demand.demand_status : demand.payment_status;
    if (!backendOnline) {
      ...
      if (control && "value" in control) control.value = originalValue || "";
      return;
    }
  ...
  function demandStatusUpdatePayload(selectedStatus, demand = {}) {
    const today = localDateString();
    if (selectedStatus === "Finalizada" || selectedStatus === "Entregue") {
      return {
        label: "OS finalizada e marcada como entregue. Ela não será mais tratada como fora do prazo.",
        overrides: {
          demand_status: "Entregue",
          system_status: "Concluída",
          system_finished_at: demand.system_finished_at || today,
        },
      };
    }
    if (selectedStatus === "Cancelada") {
      return {
        label: "OS cancelada. Ela deixa de contar como pendência operacional de prazo.",
        overrides: {
          demand_status: "Cancelada",
          system_status: "Cancelada",
          system_finished_at: demand.system_finished_at || today,
        },
      };
    }
    if (selectedStatus === "Não entregue") {
      return {
        label: "OS marcada como não entregue.",
        overrides: {
          demand_status: "Não entregue",
          system_status: demand.system_status === "Concluída" ? "Pendente" : demand.system_status,
          system_finished_at: null,
        },
      };
    }
    return {
      label: "Status da OS atualizado.",
      overrides: {
        demand_status: selectedStatus,
        system_status: demand.system_status === "Concluída" ? "Pendente" : demand.system_status,
        system_finished_at: null,
      },
    };
  ...
  function quickActionPayload(action) {
    ...
    const actions = {
      ...
      system_finished: {
        label: "Demanda finalizada e marcada como entregue.",
        overrides: {
  ...
  tableBody.addEventListener("change", (event) => {
    ...
    const demandStatusSelect = event.target.closest("[data-update-demand-status]");
    if (!demandStatusSelect) return;
    const demand = demands.find((item) => item.id === demandStatusSelect.dataset.updateDemandStatus);
    const action = demandStatusUpdatePayload(demandStatusSelect.value, demand);
    quickUpdateDemand(demandStatusSelect.dataset.updateDemandStatus, action.overrides, demandStatusSelect, action.label);
  });
  ...
  evaluationDemandMatches.addEventListener("change", (event) => {
    const demandStatusSelect = event.target.closest("[data-update-search-demand-status]");
    if (!demandStatusSelect) return;
    const demand = evaluationSearchResults.find((item) => item.id === demandStatusSelect.dataset.updateSearchDemandStatus);
    const action = demandStatusUpdatePayload(demandStatusSelect.value, demand);
    quickUpdateDemand(demandStatusSelect.dataset.updateSearchDemandStatus, action.overrides, demandStatusSelect, action.label);
  });
